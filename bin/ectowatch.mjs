#!/usr/bin/env node
/** Entry point. `npm start`, or `node bin/ectowatch.mjs --port 9000 --backfill`. */
...
if (values.help) {
  console.log(`ectowatch — local Guild Wars 1 price dashboard
...
try {
  const { server, store, poller, backup } = await startServer({
    ...
    // `node bin/ectowatch.mjs` and a Docker container behave identically
  ...
  console.error(`ectowatch: ${error.message}`);
  process.exit(1);
}
